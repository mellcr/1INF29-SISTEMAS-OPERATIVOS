...
int sizeh() {
    return sizeof(Header); //solo devolvemos el tamanho
}

unsigned size(void *ptr) {
    Header *header_ptr = (Header *)ptr - 1; // Sabemos que ptr es el puntero al bloque después del Header
    return header_ptr->s.size; 
}

void xprintq() {
    Header *current = freep;
    if (current == NULL) {
        printf("Lista de bloques libres está vacía.\n");
        return;
    }
    do {
        printf("Bloque libre en: %p, tamaño: %lu headers\n", (void *)current, current->s.size);
        //current = current->next; nos movemos al siguiente
        current = current->s.ptr;
    } while(current != freep); // Sabemos que la lista es circular
}